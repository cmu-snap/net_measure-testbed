...
def iperf3_server(port):
	...
	"""
	try:
		subprocess.check_call(f'docker exec iperf3 -s -p {port} &', shell=True)
	except subprocess.CalledProcessError as e:
		print(f"Command {e.cmd} returned non-zero exit status: {e.returncode}")


def iperf3_client(server_ip, port):
	"""
	...
	"""
	try:
		subprocess.check_call(f"docker exec {node_name} iperf3 -c {server_ip} -p {port} &", shell=True)
	except subprocess.CalledProcessError as e:
		print(f"Command {e.cmd} returned non-zero exit status: {e.returncode}")
